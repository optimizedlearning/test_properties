...
def main():
    ...
    wandb.init(project='clean_properties_run', config=args, name=args.name)
        ...
        for step, batch in enumerate(active_dataloader):
            ...
            with accelerator.accumulate(model):
                ...
                if step >0:
                    ...
                    # exp_avg_gap_1 = 0.99*exp_avg_gap_1 + (1-0.99)*(loss.detach().float() - current_loss - linear_approx )
                    # exp_avg_gap_2 = 0.9999*exp_avg_gap_2 + (1-0.9999)*(loss.detach().float() - current_loss - linear_approx )
                    # exp_avg_L_1 = 0.99*exp_avg_L_1+ (1-0.99)*compute_smoothness(model, current_param, current_grad)
                    # exp_avg_L_2 = 0.9999*exp_avg_L_2+ (1-0.9999)*compute_smoothness(model, current_param, current_grad)
                    # print("denom", denom)
