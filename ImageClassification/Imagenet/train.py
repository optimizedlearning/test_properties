...
def train_step(epoch, net, trainloader, criterion, optimizer, device,prev_loss):
    '''
    Train single epoch.
    '''
    ...
    # optimizer.save_param()
    # return stats
    return train_loss, train_acc, convexity_gap/step, L, num/denom
...
def train(net, trainloader, testloader, epochs, criterion, optimizer, scheduler, device,args, opt_loss):
    ...
    for epoch in range(epochs):
        name = 'cifar10_resnet_checkpoint/'+ str(epoch+1) + ".pth.tar"
        saved_checkpoint = torch.load(name)
        optimizer.save_param(saved_checkpoint['state_dict'])
        prev_loss = saved_checkpoint['current_loss']
        train_loss, train_acc, convexity_gap, smoothness_constant, ratio= train_step(epoch, net, trainloader, criterion, optimizer, device, prev_loss =prev_loss)
        # inner_product = inner
        if args.save:
